...
const WIDGET_ARRAY      = 'array';
const WIDGET_FILE		= 'file';
const WIDGET_SELECTFILE = 'selectfile';
const WIDGET_SIZE       = 'size';
const WIDGET_COORDS     = 'coords';
const WIDGET_COLOR      = 'color';
const WIDGET_CHAR       = 'char';
const WIDGET_TAB        = 'tab';
const WIDGET_BTN        = 'button';
const WIDGET_PLAINCODE  = 'plaincode';
...
function sqlFormat($type, $value = '', $quote = false){ //echo $type;
    switch($type){
        case DB_BOOL:
        case DB_INT: $value = intval($value);
            break;

        case DB_CHAR:
        case DB_TEXT: $value =  parseString($value);
            break;

        case DB_FLOAT: $value = floatval($value);
            break;

        case DB_PASS : $value = md5($value);
            break;

        case DB_ARRAY: $value = serialize($value);
            break;

        case DB_DATE: if($value=='') $value = date("Y-m-d H:i:s"); else{
...
function getImg($img, $dir) {
    return  BASEFMURL . GFXDIR . $dir .'/' . $img;
}

function getthumb($img, $dir = '', $echo = true){
    $dir  = GFXDIR . $dir;
...
function img($gid, $iid, $echo = false) {
    $gals = cache('galleries');
    $slug = $gals[$gid]['slug'];
    $imgs = cache('gal_' . $gid);
    $img = $imgs[$iid];
...
function installationCheck() { if(!DEV) return;
...
function drawBtns($buttons, $classname = '',  $params = array()) {
    $html = '';
